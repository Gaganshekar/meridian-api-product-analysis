...
1. Pagination issue - the biggest problem
   ...
   I consider this the most serious issue because it can affect the whole dataset without causing an obivous error.

2. refunded is not docuumented as a status
   The documentation lists four possible statuses: pending, shipped, delivered, and cancelled.
   But in orders_page2.json contains ord_1003 with status refuded.
...
3. Money is not always returned in the documented format
   ...
   3.shipping: 5.9
   4.total: 53.62
   The calculation itself is correct: $44.00 + $3.63 + $5.99 = $53.62. The problem is that the format dose not follow the documentation.
...
4. Customer email can be missing
   ...
   This could cause problem for systems that depend on an email being available for customer matching, notificationsm or other processing.
...
One order, ord_1003, has a reefunded status, but the API does not tell us how much was refunded or how refunds should be treated in the revenue calculation.
There is also the pagiination issue. If a client stopped after page 1 because has_more was false, it would calculate $249.94 and miss $79.90 from page 2.
So my conculsion is: 
...
Subject Re: Revenus reconciliation
...
Impact
an integration following the documentation can silently miss orders and produce incorrect financail reports.
what to check:
